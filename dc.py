...
def _one_runner_run(runner: OneRunnerState, conf: ESConfig) -> Tuple[OneRunnerState, Dict, jnp.ndarray]:
    """
...
def _runners_run(runners_state: RunnersState, conf: ESConfig, update_normalizer : bool = True) -> Tuple[RunnersState, Dict]:
    """
    ...
    updated_runners_batched, metrics_batched, obs_batched = jax.vmap(
        _one_runner_run, in_axes=(0, None))(runners_with_keys, conf)
...
def _generate_offspring(runners: RunnersState, conf: ESConfig, master_key: Any):
    ...
    # split key
    keys = jax.random.split(master_key, num_offspring + 1)
    new_master_key = keys[0]

    # sample parent indices and keys for alpha if needed
    pair_key = jax.random.split(new_master_key, 1)[0]  # or use another split if desired
    parent_idx = jax.random.randint(pair_key, shape=(num_offspring, 2), minval=0, maxval=num_runners)

    # get parent params
    # generate per-offspring alpha keys from per_keys (or use separate RNG)
    alpha_keys = jax.random.split(new_master_key, num_offspring)  # another source
...
def _runners_crossover_selection(runners: RunnersState, conf: ESConfig) -> Tuple[RunnersState, Dict]:
    ...
    # num_offspring = offspring_params.shape[0]
...
def main(conf):
    ...
    # Naming
    conf = OmegaConf.merge({
        "project_name": f"DC-SNN-{conf.task}",
        "run_name":     f"DC {conf.seed} {conf.network_type} {time.strftime('%H:%M %m-%d')}"
    }, conf)
    # ES Config
